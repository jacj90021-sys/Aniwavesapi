...
import { mediaProxyEnabled, mediaProxyBase } from "../lib/anime/media-proxy.js";
import { redisEnabled } from "../lib/anime/cache.js";
...
router.get("/health", async (_req, res) => {
  ...
  res.json({
    ...
    mediaProxy: {
      enabled: mediaProxyEnabled(),
      base: mediaProxyEnabled() ? mediaProxyBase().slice(0, 60) : null,
      mode: mediaProxyEnabled() ? "cloudflare" : "render-fallback",
      note: mediaProxyEnabled()
        ? "Video bytes go through Cloudflare Worker — Render egress ~0"
        : "CF_MEDIA_PROXY_URL unset — /api/proxy on Render will cost bandwidth",
    },
    redis: {
      enabled: redisEnabled(),
      note: redisEnabled()
        ? "Upstash Redis metadata cache active"
        : "In-memory cache only (set UPSTASH_REDIS_REST_URL + TOKEN)",
    },
  });
});
