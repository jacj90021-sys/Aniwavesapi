...
import {
  ...
  proxyAssetUrl,
  refererForUrl,
} from "../lib/anime/media-proxy.js";
import {
  cacheGetAsync,
  cacheSetAsync,
  TTL,
  redisEnabled,
} from "../lib/anime/cache.js";
import type { StreamSource } from "../lib/anime/types.js";
 ...
 * proxy: optional scrape proxy URL (not media)
 *
 * Bandwidth design:
 *   Render only returns JSON (m3u8 URL + proxiedM3u8 pointing at Cloudflare Worker).
 *   The browser loads HLS segments from CF_MEDIA_PROXY_URL — never through Render.
 *   /api/proxy on this host is emergency fallback only.
 */
function safeProxyFallback(targetUrl: string, referer: string, origin: string): string {
  // Absolute same-origin /api/proxy — always works, burns Render egress but never 500s.
  const base = origin.replace(/\/+$/, "");
  return `${base}/api/proxy?url=${encodeURIComponent(targetUrl)}&referer=${encodeURIComponent(referer)}`;
}

async function buildProxiedM3u8(
  targetUrl: string,
  origin: string
): Promise<{ proxiedM3u8: string; viaWorker: boolean }> {
  try {
    if (mediaProxyEnabled()) {
      const url = buildMediaProxyUrl(targetUrl, { referer: refererForUrl(targetUrl) });
      if (url && url.length > 0) return { proxiedM3u8: url, viaWorker: true };
    }
  } catch (e) {
    req_log_fallback(e);
  }
  return {
    proxiedM3u8: safeProxyFallback(targetUrl, refererForUrl(targetUrl), origin),
    viaWorker: false,
  };
}

function req_log_fallback(_e: unknown) {
...
router.get("/stream", async (req, res): Promise<void> => {
  ...
  try {
    ...
    req.log.info(
      {
        serverId: serverId.slice(0, 40),
        mediaProxy: mediaProxyEnabled(),
        redis: redisEnabled(),
      },
      "stream requested via serverId"
    );

    // Metadata cache (Redis/memory) — avoids re-running PoW/extraction on repeat plays.
    let cached: (StreamSource & { proxiedM3u8?: string }) | undefined;
    try {
      cached = await cacheGetAsync<StreamSource & { proxiedM3u8?: string }>(cacheKey);
    } catch {
      cached = undefined;
    }
    if (cached?.m3u8) {
      const { proxiedM3u8, viaWorker } = await buildProxiedM3u8(cached.m3u8, origin);
      const subtitles = (cached.subtitles ?? []).map((s) => ({
        ...s,
        url: proxyAssetUrl(s.url) ?? s.url,
      }));
      const thumbnails = proxyAssetUrl(cached.thumbnails);
      req.log.info({ serverId: serverId.slice(0, 40), cache: true }, "stream cache hit");
      res.json({
        ...cached,
        subtitles,
        thumbnails,
        proxiedM3u8,
        mediaProxy: viaWorker ? "cloudflare" : "render-fallback",
        _cached: true,
        _server: "direct",
      });
      return;
    ...
    if (stream?.m3u8) {
      const { proxiedM3u8, viaWorker } = await buildProxiedM3u8(stream.m3u8, origin);
      const subtitles = (stream.subtitles ?? []).map((s) => ({
        ...s,
        url: proxyAssetUrl(s.url) ?? s.url,
      }));
      const thumbnails = proxyAssetUrl(stream.thumbnails);

      try {
        await cacheSetAsync(cacheKey, stream, TTL.STREAM);
      } catch {
      ...
      if (!mediaProxyEnabled()) {
        req.log.warn(
          "CF_MEDIA_PROXY_URL unset — video is proxied via /api/proxy on Render (bandwidth cost). Deploy cf_media_worker.js and set the env var."
        );
      }
    ...
    // NEVER raw-500. Log it, return a clean JSON error so the player can surface it.
    req.log.error({ err: err instanceof Error ? err.message : String(err), stack: err instanceof Error ? err.stack : undefined }, "stream handler crashed");
    if (!res.headersSent) {
      res.status(500).json({ error: "Stream handler error", detail: err instanceof Error ? err.message : "unknown" });
    }
  }
});
 ...
 *
 * EMERGENCY / local-dev fallback only.
 * Every byte here costs Render egress. Production clients must use
 * CF_MEDIA_PROXY_URL (see cf_media_worker.js) via proxiedM3u8 from /api/stream.
 */
