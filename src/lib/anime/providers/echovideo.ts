...
import { extractViaPythonScraper } from "./python-scraper.js";
...
export async function extractEchovideo(
  ...
  if (!sourceId) {
    ...
    return extractViaPythonScraper(embedUrl, "echovideo", skipData);
  }
  ...
  try {
    ...
    logger.error(
      ...
      "[Echovideo S2] FAILED — getSources request failed, falling back to Python scraper"
    );
    return extractViaPythonScraper(embedUrl, "echovideo", skipData);
  }
  ...
  if (!m3u8) {
    logger.warn(
      ...
      "[Echovideo S3] no m3u8 in sources — falling back to Python scraper"
    );
    return extractViaPythonScraper(embedUrl, "echovideo", skipData);
  }
